...
class DQN(object):
    ...
    def train(self, replay_buffer):
        ...
        with torch.no_grad():
            target_Q_steer = reward + (1 - done) * self.discount * self.Q_target(next_state)[0].max(1, keepdim=True)[0]
            target_Q_brake = reward + (1 - done) * self.discount * self.Q_target(next_state)[1].min(1, keepdim=True)[0]
            target_Q_throttle = reward + (1 - done) * self.discount * self.Q_target(next_state)[2].max(1, keepdim=True)[
                0]

        # Get current Q estimate
        current_Q_steer, current_Q_brake, current_Q_throttle = self.Q(state)

        steer = steer.to(torch.int64)
        brake = brake.to(torch.int64)
        throttle = throttle.to(torch.int64)

        current_Q_steer = current_Q_steer.gather(1, steer)
        current_Q_brake = current_Q_brake.gather(1, brake)
        current_Q_throttle = current_Q_throttle.gather(1, throttle)
