...
def calculate_metrics_means(file_path):
    # Carica i dati
    ...
    metrics_means_latex = "Media & {:.1f} & {:.1f} & {:.1f} & {:.1f} & {:.1f} &  {:.1f} \\\\".format(
        means['reward'], means['duration'], means['timesteps'], means['collisions'],
        means['lane_invasions'], means['avg_speed']
    )

    print(metrics_means_latex)

calculate_metrics_means("primotestnuovamappa.csv")
